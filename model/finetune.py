...
class SummarizationModule(BaseTransformer):
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        outputs = self(src_ids, attention_mask=src_mask, decoder_input_ids=decoder_input_ids, use_cache=False, output_attentions=True, output_hidden_states=True)

        sen_token_id = self.tokenizer.mask_token_id # 50265
        print("sen_token_id:", sen_token_id)
        ...
        if self.hparams.label_smoothing == 0:
            ...
            loss_mask = (batch["normal_labels"][:, None] * torch.ones_like(tgt_ids) * (1 - tgt_ids.eq(pad_token_id).to(torch.float))).view(-1)

            ce_loss_fct = torch.nn.CrossEntropyLoss(ignore_index=pad_token_id, reduction="none")
            assert lm_logits.shape[-1] == self.vocab_size
            loss = torch.sum(ce_loss_fct(lm_logits.view(-1, lm_logits.shape[-1]), tgt_ids.view(-1)) * loss_mask) / (torch.sum(loss_mask)+1e-20)
        else:
        ...
        # coeff = 1. / (float(args.reorder) + float(args.sbert))
        ...
        if args.reorder:
            ...
            sen_att_logits = torch.matmul(self.reorder_linear_layer(sent_hidden_states), torch.transpose(sent_hidden_states, 1, 2))
            ...
            reorder_mask = (1 - torch.eye(sentence_num)[None, :, :].to(batch["type_labels"].device)) * \
                (torch.eq(batch["type_labels"], 0) | torch.eq(batch["type_labels"], 1)).to(torch.float)[:, None, None]
            ...
            batch_reorder_loss = -torch.log(pred_score+1e-20) * reorder_label_matrix - torch.log(1-pred_score+1e-20) * (1-reorder_label_matrix)

            batch_reorder_loss *= reorder_mask
            reorder_loss = torch.mean(torch.sum(batch_reorder_loss, [1,2]) / (torch.sum(reorder_mask, [1,2]) + 1e-20) * (1 - 0.99 * batch["normal_labels"]))
            loss += reorder_loss
        ...
        if args.sbert:
            ...
            batch_sbert_loss = torch.max(torch.abs(pred_score - sbert_score_label) - 0.1, torch.zeros_like(pred_score).to(pred_score.device))
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument(
            ...
            default=1000,  # these defaults are optimized for CNNDM. For xsum, see README.md.
            type=int,
            help="The maximum total input sequence length after tokenization. Sequences longer "
            "than this will be truncated, sequences shorter will be padded.",
        )
        ...
        parser.add_argument("--attention_sent", action="store_true")
