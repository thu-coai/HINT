...
class AbstractSeq2SeqDataset(Dataset):
    def __init__(
        ...
        **dataset_kwargs
        ...
        self.order_file = Path(data_dir).joinpath(type_path + "_order.target")
        self.sbert_score_file = Path(data_dir).joinpath(type_path + "_sbertscore.target")
...
class Seq2SeqSbertscoreOrderDisDataset(AbstractSeq2SeqDataset):
    """A dataset that calls prepare_seq2seq_batch."""
    ...
    def __getitem__(self, index) -> Dict[str, str]:
        ...
        source_line = self.prefix + linecache.getline(str(self.src_file), index).rstrip("\n")
        tgt_line = linecache.getline(str(self.tgt_file), index).rstrip("\n")
        score_line = linecache.getline(str(self.sbert_score_file), index).rstrip("\n")
        order_line = linecache.getline(str(self.order_file), index).rstrip("\n")
        assert source_line, f"empty source line for index {index}"
        assert tgt_line, f"empty tgt line for index {index}"
        return {"tgt_texts": tgt_line, "src_texts": source_line, "id": index - 1, "score": score_line, "order": order_line}
